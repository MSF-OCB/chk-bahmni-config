SELECT
		...
		IFNULL(SessDemandee.User,'') AS 'Sess. Demandee Par',
	...
	INNER JOIN users u ON u.user_id = session_name.creator
	LEFT JOIN 
		(
			SELECT 
				v.patient_id,
			...
			INNER JOIN users u ON u.user_id = requestSessType.creator
			WHERE requestSessType.name IN ('SessionPSEDemande')
			AND requestSessType.locale = 'fr'
			GROUP BY patient_id,visit_id ) AS SessDemandee ON  v.visit_id = SessDemandee.visit_id
