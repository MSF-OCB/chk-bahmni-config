SELECT pi.identifier                                                                           AS "ID Patient",
       group_concat(DISTINCT
                    (CASE WHEN pat.name = 'Type de cohorte' THEN cn.name ELSE NULL END))       AS "TypeCohorte",
       ...
       (CASE
          WHEN lig.Ligne IN ("1ere", "2e", "3e", "1ere alternative", "2e alternative", "3e alternative", "Autres")
                  THEN lig.Ligne
...
FROM (/* get date de admission details for each visit of patients  */
     ...
     FROM (SELECT o2.person_id,
                                 ...
                                 o3.voided IS FALSE
                                   AND o3.concept_id = (SELECT concept_id
                                                        ...
                                                        WHERE cn2.name IN ("IPD Admission, Date d'admission")
                                                          ...
                                                          AND cn2.locale = 'fr') AND
                                 DATE(o3.value_datetime) BETWEEN DATE('#startDate#') and Date('#endDate#'))AS admdate
       ...
       LEFT JOIN person_attribute pa ON admdate.person_id = pa.person_id AND pa.voided = 0
       ...
       LEFT JOIN (/* get structure de référence details for each visit of patients */
         ...
         ON refer.person_id = admdate.person_id AND refer.visit = admdate.visitid
       LEFT JOIN (/* get mode d'entrée details for latest encounter of each visit of patients  */
         ...
         ON syndrome.person_id = admdate.person_id AND syndrome.visit = admdate.visitid
       LEFT JOIN (/* get malade mort details on arrival*/
                 ...
                 FROM obs obsForActivityStatus
                        ...
                        INNER JOIN concept_view cn1 ON obsForActivityStatus.concept_id = cn1.concept_id
                        INNER JOIN concept_view cv ON cv.concept_id = obsForActivityStatus.concept_id AND
                                                      cv.concept_full_name = "Malade arrivé mort"
                        INNER JOIN concept_answer ca
                          ON ca.concept_id = cv.concept_id AND obsForActivityStatus.value_coded = ca.answer_concept AND
                             cv.concept_full_name = "Malade arrivé mort"
                               AND obsForActivityStatus.voided = 0) AS malade
         ON malade.person_id = admdate.person_id AND malade.visit = admdate.visitid
       LEFT JOIN (/* get fosa details for latest encounter of each visit of patients  */
                 ...
                 FROM obs obsForActivityStatus
                        ...
                        INNER JOIN concept_view cn1 ON obsForActivityStatus.concept_id = cn1.concept_id
                        INNER JOIN concept_view cv ON cv.concept_id = obsForActivityStatus.concept_id AND
                                                      cv.concept_full_name = "FOSA de suivi ARV"
                        INNER JOIN concept_answer ca
                          ON ca.concept_id = cv.concept_id AND obsForActivityStatus.value_coded = ca.answer_concept AND
                             cv.concept_full_name = "FOSA de suivi ARV"
                               AND obsForActivityStatus.voided = 0) AS fosa
         ON fosa.person_id = admdate.person_id AND fosa.visit = admdate.visitid
       LEFT JOIN (/* get Hospi antérieures - date d'admission 1 details for latest encounter of each visit of patients  */
                       ...
                       FROM (SELECT MAX(o.encounter_id) AS latestEncounter, o.person_id, o.concept_id, e.visit_id
                             FROM obs o
                                    INNER JOIN concept_name cn ON o.concept_id = cn.concept_id
                                                                    AND
                                                                  cn.name IN ('IPD Admission Section, Hospitalisations antérieures')
                       ...
                       GROUP BY latestVisitEncounterAndVisitForConcept.visit_id) firstAddSectionDateConceptInfo
                        INNER JOIN obs o3
                          ON o3.obs_group_id = firstAddSectionDateConceptInfo.firstAddSectionObsGroupId AND
                             o3.voided IS FALSE AND
                             o3.concept_id = (SELECT concept_id
                                              FROM concept_name cn2
                                              WHERE cn2.name in ('IPD Admission, Date admission')
                                                AND cn2.voided IS FALSE
                                                AND cn2.concept_name_type = 'FULLY_SPECIFIED'
                                                AND cn2.locale = 'fr')) AS hosp1
         ON hosp1.person_id = admdate.person_id AND hosp1.visit = admdate.visitid
       LEFT JOIN (/* get Hospi antérieures - date d'admission 2 details for latest encounter of each visit of patients  */
                 ...
                 FROM (SELECT firstAddSectionDateConceptInfo.person_id,
                       ...
                       FROM (SELECT o2.person_id,
                             ...
                             FROM (SELECT MAX(o.encounter_id) AS latestEncounter, o.person_id, o.concept_id, e.visit_id
                                   FROM obs o
                                          INNER JOIN concept_name cn ON o.concept_id = cn.concept_id AND
                                                                        cn.name IN ('IPD Admission Section, Hospitalisations antérieures') AND
                             ...
                             GROUP BY latestVisitEncounterAndVisitForConcept.visit_id) firstAddSectionDateConceptInfo
                              INNER JOIN obs o3 ON o3.obs_id > firstAddSectionDateConceptInfo.firstAddSectionObsGroupId
                                                     AND
                                                   o3.concept_id = firstAddSectionDateConceptInfo.concept_id
                                                     AND
                       ...
                       GROUP BY firstAddSectionDateConceptInfo.visit_id) secondAddSectionDateConceptInfo
                        INNER JOIN obs o3 ON o3.obs_group_id =
                                             secondAddSectionDateConceptInfo.secondAddSectionObsGroupId AND
                                             o3.voided IS FALSE AND
                                             o3.concept_id =
                                             (SELECT concept_id
                                              FROM concept_name cn2
                                              WHERE cn2.name in ('IPD Admission, Date admission')
                                                AND cn2.voided IS FALSE
                                                AND cn2.concept_name_type = 'FULLY_SPECIFIED'
                                                AND cn2.locale = 'fr')) AS hosp2
         ON hosp2.person_id = admdate.person_id AND hosp2.visit = admdate.visitid
       LEFT JOIN (/* get Hospi antérieures - date d'admission 3 details for latest encounter of each visit of patients  */
                 SELECT thirdAddSectionDateConceptInfo.person_id,
                        thirdAddSectionDateConceptInfo.visit_id AS visit,
                        o3.value_datetime                       AS NAME
                 FROM (SELECT secondAddSectionDateConceptInfo.person_id,
                       ...
                       FROM (SELECT firstAddSectionDateConceptInfo.person_id,
                             ...
                             GROUP BY firstAddSectionDateConceptInfo.visit_id) secondAddSectionDateConceptInfo
                              INNER JOIN obs o3
                                ON o3.obs_id > secondAddSectionDateConceptInfo.secondAddSectionObsGroupId AND
                                   o3.concept_id = secondAddSectionDateConceptInfo.concept_id AND
                                   o3.encounter_id = secondAddSectionDateConceptInfo.latestEncounter AND
                                   o3.voided IS FALSE
                       GROUP BY secondAddSectionDateConceptInfo.visit_id) thirdAddSectionDateConceptInfo
                        INNER JOIN obs o3 ON o3.obs_group_id =
                                             thirdAddSectionDateConceptInfo.thirdAddSectionObsGroupId AND
                                             o3.voided IS FALSE AND
                                             o3.concept_id =
                                             (SELECT concept_id
                                              FROM concept_name cn2
                                              WHERE cn2.name in ('IPD Admission, Date admission')
                                                AND cn2.voided IS FALSE
                                                AND cn2.concept_name_type = 'FULLY_SPECIFIED'
                                                AND cn2.locale = 'fr')) AS hosp3
         ON hosp3.person_id = admdate.person_id AND hosp3.visit = admdate.visitid
       LEFT JOIN (/* get Stade OMS details for latest encounter of each visit of patients  */
                 ...
                 WHERE obsForActivityStatus.concept_id IN
                       (SELECT concept_id FROM concept_view WHERE concept_full_name = "Stade clinique OMS")
                   AND obsForActivityStatus.value_coded IN (SELECT answer_concept
                                                            FROM concept_answer
                                                            WHERE concept_id =
                                                                  (SELECT concept_id
                                                                   FROM concept_view
                                                                   WHERE concept_full_name IN ("Stade clinique OMS")))
                   AND obsForActivityStatus.voided = 0) AS stade
         ON stade.person_id = admdate.person_id AND stade.visit = admdate.visitid
       LEFT JOIN (/* get Glasgow details for latest encounter of each visit of patients  */
                 ...
                 WHERE obsq.concept_id IN (SELECT concept_id
                                           ...
                                           WHERE NAME = "IPD Admission, SNC Glasgow(/15)"
                                             AND concept_name_type = 'FULLY_SPECIFIED'
                                             AND locale = 'fr')
                   AND obsq.voided = 0) AS glas ON glas.personid = admdate.person_id AND glas.visit = admdate.visitid
       LEFT JOIN (/* get genexpert details for latest encounter of each visit of patients  */
                 ...
                 FROM obs oTest
                        ...
                        INNER JOIN concept_view cvt ON cvt.concept_id = oTest.concept_id AND cvt.concept_full_name IN
                 ...
                 GROUP BY visit_id) AS genexpert
         ON genexpert.patient_id = admdate.person_id AND genexpert.visit = admdate.visitid
                 ...
                 FROM obs o
                        ...
                        INNER JOIN concept_name answer_concept
                          ON o.value_coded = answer_concept.concept_id AND answer_concept.voided IS FALSE AND
                             ...
                             answer_concept.locale = 'fr') AS TBLAM
         ON TBLAM.person_id = admdate.person_id AND admdate.visitid = TBLAM.visitid
       LEFT JOIN (/* get Syphilis details for latest encounter of each visit of patients  */
                 ...
                 FROM obs oTest
                        ...
                        INNER JOIN concept_view cvt
                          ON cvt.concept_id = oTest.concept_id AND cvt.concept_full_name = 'Syphilis' AND
                             oTest.voided IS FALSE AND
                             cvt.retired IS FALSE
                        INNER JOIN concept_view cvta ON cvta.concept_id = oTest.value_coded AND cvta.retired IS FALSE
                 GROUP BY et.visit_id) AS syp ON syp.patient_id = admdate.person_id AND syp.visitid = admdate.visitid
       LEFT JOIN (/* get crag details for latest encounter of each visit of patients  */SELECT et.visit_id                 AS visitid,
                                                                                               et.patient_id,
                                                                                        ...
                                                                                        FROM obs oTest
                                                                                               ...
                                                                                               INNER JOIN concept_view cvt
                                                                                                 ON cvt.concept_id = oTest.concept_id AND
                                                                                                    cvt.concept_full_name = 'Crag' AND
                                                                                                    oTest.voided IS FALSE AND
                                                                                                    cvt.retired IS FALSE
                                                                                               INNER JOIN concept_view cvta
                                                                                                 ON cvta.concept_id = oTest.value_coded AND cvta.retired IS FALSE
                                                                                        GROUP BY et.visit_id) AS crag
         ON crag.patient_id = admdate.person_id AND crag.visitid = admdate.visitid
       LEFT JOIN (/* get Crag serique details for latest encounter of each visit of patients  */
                 SELECT et.visit_id AS visitid, et.patient_id, MAX(cvta.concept_full_name) AS VALUE
                 FROM obs oTest
                        INNER JOIN encounter et ON et.encounter_id = oTest.encounter_id AND et.voided IS FALSE
                        INNER JOIN concept_view cvt
                          ON cvt.concept_id = oTest.concept_id AND cvt.concept_full_name = 'Crag serique' AND
                             oTest.voided IS FALSE AND
                             cvt.retired IS FALSE
                        INNER JOIN concept_view cvta ON cvta.concept_id = oTest.value_coded AND cvta.retired IS FALSE
                 GROUP BY et.visit_id) AS crag2
         ON crag2.patient_id = admdate.person_id AND crag2.visitid = admdate.visitid
       LEFT JOIN (/* get hep B details for latest encounter of each visit of patients  */
                 SELECT et.visit_id AS visitid, et.patient_id, MAX(cvta.concept_full_name) AS VALUE
                 FROM obs oTest
                        INNER JOIN encounter et ON et.encounter_id = oTest.encounter_id AND et.voided IS FALSE
                        INNER JOIN concept_view cvt
                          ON cvt.concept_id = oTest.concept_id AND cvt.concept_full_name = 'Hépatite B' AND
                             oTest.voided IS FALSE AND
                             cvt.retired IS FALSE
                        INNER JOIN concept_view cvta ON cvta.concept_id = oTest.value_coded AND cvta.retired IS FALSE
                 GROUP BY et.visit_id) AS hep ON hep.patient_id = admdate.person_id AND hep.visitid = admdate.visitid
       LEFT JOIN (/* get hemoglobline details for latest encounter of each visit of patients  */SELECT hemo_obs.vid     AS vid,
                                                                                                ...
                                                                                                FROM (SELECT v.visit_id         AS vid,
                                                                                                      ...
                                                                                                      FROM visit v
                                                                                                             ...
                                                                                                             INNER JOIN (SELECT o.encounter_id,
                                                                                                                                ...
                                                                                                                                o.concept_id
                                                                                                                         FROM obs o
                                                                                                                                INNER JOIN concept_view hemo_val
                                                                                                                                  ON hemo_val.concept_id = o.concept_id AND
                                                                                                                                     o.voided IS FALSE AND
                                                                                                                                     hemo_val.retired IS FALSE AND
                                                                                                                                     hemo_val.concept_full_name IN ('Hémoglobine (Hemocue)(g/dl)') AND
                                                                                                                          ...
                                                                                                                          FROM obs o
                                                                                                                                 INNER JOIN concept_view hemo_val
                                                                                                                                   ON hemo_val.concept_id = o.concept_id AND
                                                                                                                                      o.voided IS FALSE AND
                                                                                                                                      hemo_val.retired IS FALSE AND
                                                                                                                                      hemo_val.concept_full_name IN ('Hemoglobine') AND
                                                                                                                                      o.value_numeric IS NOT NULL)) hemo
                                                                                                               ON hemo.encounter_id = e.encounter_id
                                                                                                      ORDER BY obsdate DESC) hemo_obs
                                                                                                GROUP BY vid) AS hemo
         ON hemo.PID = admdate.person_id AND hemo.vid = admdate.visitid
                 ...
                 FROM (SELECT v.visit_id         AS vid,
                       ...
                       FROM visit v
                              ...
                              INNER JOIN (SELECT o.encounter_id,
                                                 ...
                                                 o.concept_id
                                          FROM obs o
                                                 INNER JOIN concept_view glyc_val
                                                   ON glyc_val.concept_id = o.concept_id AND o.voided IS FALSE AND
                                                      glyc_val.retired IS FALSE AND
                                                      glyc_val.concept_full_name IN ('Glycémie(mg/dl)') AND
                                                      o.value_numeric IS NOT NULL
                                          UNION
                                          (SELECT o.encounter_id,
                                                  o.person_id,
                                           ...
                                           FROM obs o
                                                  INNER JOIN concept_view glyc_val
                                                    ON glyc_val.concept_id = o.concept_id AND o.voided IS FALSE AND
                                                       glyc_val.retired IS FALSE AND
                                                       glyc_val.concept_full_name IN ('Glycémie') AND
                                                       o.value_numeric IS NOT NULL)) glyc
                                ON glyc.encounter_id = e.encounter_id
                       ORDER BY obsdate DESC) glyc_obs
                 GROUP BY vid) AS gly ON gly.PID = admdate.person_id AND gly.vid = admdate.visitid
       LEFT JOIN (/* get creatinine details for latest encounter of each visit of patients  */SELECT et.visit_id              AS visitid,
                                                                                                     et.patient_id,
                                                                                              ...
                                                                                              FROM obs oTest
                                                                                                     ...
                                                                                                     INNER JOIN concept_view cvt
                                                                                                       ON cvt.concept_id = oTest.concept_id AND
                                                                                                          cvt.concept_full_name = 'Creatinine' AND
                                                                                                          oTest.voided IS FALSE AND
                                                                                                          cvt.retired IS FALSE
                                                                                              GROUP BY visit_id) AS creat
         ON creat.patient_id = admdate.person_id AND creat.visitid = admdate.visitid
       LEFT JOIN (/* get GPT details for latest encounter of each visit of patients  */SELECT et.visit_id              AS visitid,
                                                                                              et.patient_id,
                                                                                              Max(oTest.value_numeric) AS VALUE
                                                                                       FROM obs oTest
                                                                                              INNER JOIN encounter et
                                                                                              ...
                                                                                              INNER JOIN concept_view cvt
                                                                                                ON cvt.concept_id = oTest.concept_id AND
                                                                                                   cvt.concept_full_name = 'GPT' AND
                                                                                                   oTest.voided IS FALSE AND
                                                                                                   cvt.retired IS FALSE
                                                                                       GROUP BY visit_id) AS gpt
         ON gpt.patient_id = admdate.person_id AND gpt.visitid = admdate.visitid
       LEFT JOIN (/* get Date diagnostic VIH(Ant) details for latest encounter of each visit of patients  */
                 ...
                 FROM (SELECT o2.person_id,
                       ...
                       FROM (SELECT MAX(o.encounter_id) AS latestEncounter, o.person_id, o.concept_id, e.visit_id
                             FROM obs o
                                    INNER JOIN concept_name cn
                                      ON o.concept_id = cn.concept_id AND cn.name IN ("Historique ARV") AND
                                         cn.voided IS FALSE AND cn.concept_name_type = 'FULLY_SPECIFIED' AND
                                         cn.locale = 'fr' AND o.voided IS FALSE
                                    INNER JOIN encounter e ON e.encounter_id = o.encounter_id AND e.voided IS FALSE
                             GROUP BY e.visit_id) latestVisitEncounterAndVisitForConcept
                       ...
                       GROUP BY latestVisitEncounterAndVisitForConcept.visit_id) firstAddSectionDateConceptInfo
                        INNER JOIN obs o3
                          ON o3.obs_group_id = firstAddSectionDateConceptInfo.firstAddSectionObsGroupId AND
                             ...
                             o3.concept_id = (SELECT concept_id
                                              FROM concept_name cn2
                                              WHERE cn2.name = ('Date diagnostic VIH(Ant)')
                                                AND cn2.voided IS FALSE
                                                AND cn2.concept_name_type = 'FULLY_SPECIFIED'
                                                AND cn2.locale = 'fr')) AS hiv
         ON hiv.person_id = admdate.person_id AND hiv.visitid = admdate.visitid
       LEFT JOIN (/* get ARV histoire details for latest encounter of each visit of patients  */
                 ...
                 FROM obs obsForActivityStatus
                        ...
                        INNER JOIN concept_view cn1 ON obsForActivityStatus.concept_id = cn1.concept_id
                        INNER JOIN concept_view cv ON cv.concept_id = obsForActivityStatus.concept_id AND
                                                      cv.concept_full_name = "IPD Admission, Histoire ARV"
                        INNER JOIN concept_answer ca
                          ON ca.concept_id = cv.concept_id AND obsForActivityStatus.value_coded = ca.answer_concept AND
                             cv.concept_full_name = "IPD Admission, Histoire ARV"
                               AND obsForActivityStatus.voided = 0) AS hist
         ON hist.person_id = admdate.person_id AND hist.visit = admdate.visitid
       LEFT JOIN (/* get Si interrompu details for latest encounter of each visit of patients  */
                 ...
                 FROM obs obsForActivityStatus
                        ...
                        INNER JOIN concept_view cn1 ON obsForActivityStatus.concept_id = cn1.concept_id
                        INNER JOIN concept_view cv ON cv.concept_id = obsForActivityStatus.concept_id AND
                                                      cv.concept_full_name = "IPD Admission, Si interrompu"
                        INNER JOIN concept_answer ca
                          ON ca.concept_id = cv.concept_id AND obsForActivityStatus.value_coded = ca.answer_concept AND
                             cv.concept_full_name = "IPD Admission, Si interrompu"
                               AND obsForActivityStatus.voided = 0) AS si
         ON si.person_id = admdate.person_id AND si.visitid = admdate.visitid
       LEFT JOIN (/* get ligne ARV en cours details for latest encounter of each visit of patients  */
                 ...
                 FROM obs obsForActivityStatus
                        ...
                        INNER JOIN concept_view cn1 ON obsForActivityStatus.concept_id = cn1.concept_id
                        INNER JOIN concept_view cv ON cv.concept_id = obsForActivityStatus.concept_id AND
                                                      cv.concept_full_name = "IPD Admission, Ligne ARV en cours"
                        INNER JOIN concept_answer ca
                          ON ca.concept_id = cv.concept_id AND obsForActivityStatus.value_coded = ca.answer_concept AND
                             cv.concept_full_name = "IPD Admission, Ligne ARV en cours"
                               AND obsForActivityStatus.voided = 0) AS ligne
         ON ligne.person_id = admdate.person_id AND ligne.visitid = admdate.visitid
       LEFT JOIN (/* get Arrêt des ARV details for latest encounter of each visit of patients  */
         ...
         ON aveg.person_id = admdate.person_id AND aveg.visitid = admdate.visitid
       LEFT JOIN (/* get TB en cours details for latest encounter of each visit of patients  */
                 ...
                 FROM obs obsForActivityStatus
                        ...
                        INNER JOIN concept_view cv ON cv.concept_id = obsForActivityStatus.concept_id AND
                                                      cv.concept_full_name =
                                                      "IPD Admission, TB en cours de traimtement à l'admission"
                        INNER JOIN concept_answer ca
                          ON ca.concept_id = cv.concept_id AND obsForActivityStatus.value_coded = ca.answer_concept AND
                             cv.concept_full_name = "IPD Admission, TB en cours de traitement à l'admission"
                               AND obsForActivityStatus.voided = 0) AS tben
         ON tben.person_id = admdate.person_id AND tben.visitid = admdate.visitid
       LEFT JOIN (/* get Elements de diagnostic TB details for latest encounter of each visit of patients  */
         ...
         ON elementTB.person_id = admdate.person_id AND elementTB.visitid = admdate.visitid
       LEFT JOIN (/* get date de sortie details for latest encounter of each visit of patients  */
                 SELECT firstAddSectionDateConceptInfo.person_id,
                        firstAddSectionDateConceptInfo.visit_id AS visitid,
                        o3.value_datetime                       AS NAME
                 FROM (SELECT o2.person_id,
                       ...
                       FROM (SELECT MAX(o.encounter_id) AS latestEncounter, o.person_id, o.concept_id, e.visit_id
                             FROM obs o
                                    INNER JOIN concept_name cn ON o.concept_id = cn.concept_id AND
                                                                  cn.name IN ("Formulaire de sortie") AND
                                                                  cn.voided IS FALSE AND
                                                                  cn.concept_name_type = 'FULLY_SPECIFIED' AND
                                                                  cn.locale = 'fr' AND
                                                                  o.voided IS FALSE
                       ...
                       GROUP BY latestVisitEncounterAndVisitForConcept.visit_id) firstAddSectionDateConceptInfo
                        INNER JOIN obs o3 ON o3.obs_group_id =
                                             firstAddSectionDateConceptInfo.firstAddSectionObsGroupId AND
                                             o3.voided IS FALSE AND
                                             o3.concept_id =
                                             (SELECT concept_id
                                              FROM concept_name cn2
                                              WHERE cn2.name = ('Date de sortie')
                                                AND cn2.voided IS FALSE
                                                AND cn2.concept_name_type = 'FULLY_SPECIFIED'
                                                AND cn2.locale = 'fr')) AS sortdate
         ON sortdate.person_id = admdate.person_id AND sortdate.visitid = admdate.visitid
       LEFT JOIN (/* get Programme ARV details for latest encounter of each visit of patients  */
                 SELECT patientID,
                        DateDebutARV,
                        (CASE
                           WHEN patientWithARVEnrolled.LigneARV THEN patientWithARVEnrolled.LigneARV
                           ELSE "NoPhase" END) AS "Ligne"
                 FROM (SELECT patientID,
                              (CASE
                                 WHEN phaseOfProg IN ("1ere",
                                                      "2e",
                                                      "3e",
                                                      "1ere alternative",
                                                      "2e alternative",
                                                      "3e alternative",
                                                      "Autres") THEN phaseOfProg
                                 ELSE NULL END)    AS "LigneARV",
                              phaseDate            AS "DateDebutligne",
                              DATE(enrollmentDate) AS "DateDebutARV"
                       FROM (SELECT Y.program_id, Y.patientID, Y.enrollmentDate, Y.phaseDate, Y.vid, Y.phaseOfProg
                             FROM (SELECT X.program_id, X.patientID, X.enrollmentDate, X.phaseDate, X.vid, X.phaseOfProg
                                   FROM (SELECT ARVprog.program_id                                                  AS program_id,
                                         ...
                                         FROM patient_program patientARVprog
                                                INNER JOIN program ARVprog
                                                  ON ARVprog.program_id = patientARVprog.program_id AND
                                                     patientARVprog.voided = 0
                                                LEFT JOIN patient_state patientPhaseOfProg
                                                  ON patientARVprog.patient_program_id =
                                                     patientPhaseOfProg.patient_program_id AND
                                                     patientPhaseOfProg.voided = 0
                                                ...
                                                LEFT JOIN concept_name cn
                                                  ON progWorkflowState.concept_id = cn.concept_id AND cn.voided = 0 AND
                                                     cn.locale = 'fr' AND cn.concept_name_type = 'SHORT'
                             ...
                             GROUP BY patientID) AS arv) AS patientWithARVEnrolled) AS lig
         ON lig.patientID = admdate.person_id
       LEFT JOIN (/* get Syndrome et Diagnostic details for latest encounter of each visit of patients  */
         ...
         ON SY.person_id = admdate.person_id AND SY.visitid = admdate.visitid
       LEFT JOIN (/* get Syndrome et Diagnostic 2 details for latest encounter of each visit of patients  */
         ...
         ON SY2.person_id = admdate.person_id AND SY2.visitid = admdate.visitid
       LEFT JOIN (/* get 1er Diagnostic  on admission */
         ...
         ON dg.person_id = admdate.person_id AND dg.visitid = admdate.visitid
                          ...
                          ON o3.obs_group_id = secondAddSectionDateConceptInfo.secondAddSectionObsGroupId AND
                             ...
                             o3.concept_id = (SELECT concept_id
                                              ...
                                              WHERE cn2.name IN ('Fds, Diagnostic')
                                                AND cn2.voided IS FALSE
                                                AND cn2.concept_name_type = 'FULLY_SPECIFIED'
                                                AND cn2.locale = 'fr')) dg2
         ON dg2.person_id = admdate.person_id AND dg2.visitid = admdate.visitid
       LEFT JOIN (SELECT et.visit_id              AS visitid,
                  ...
                  FROM obs oTest
                         ...
                         INNER JOIN concept_view cvt ON cvt.concept_id = oTest.concept_id AND cvt.concept_full_name IN (
                          ...
                          'Charge Virale HIV - Value') AND oTest.voided IS FALSE AND
                                                        cvt.retired IS FALSE
                  GROUP BY visit_id) AS CV ON CV.patient_id = admdate.person_id AND CV.visitid = admdate.visitid
       LEFT JOIN (/* get TB Précédentes details for each visit of patients  */
                 SELECT tbPatients.person_id          AS pid,
                        (CASE
                           WHEN ifnull(dateCreatedOnTB, '1981-01-01') > ifnull(dateCreatedForm, '1981-01-01')
                                   THEN Tbprecedents
                           ELSE TbPrecedentsForm END) AS "TbPrecedents",
                        (CASE
                           WHEN ifnull(dateCreatedOnTB, '1981-01-01') > ifnull(dateCreatedForm, '1981-01-01') THEN Anne
                           ELSE AnneDiagnostic END)   AS "Annee",
                        dateCreatedOnTB,
                        dateCreatedForm
                 FROM (SELECT person.person_id,
                       ...
                       FROM person
                                         ...
                                         FROM (SELECT X.patientID,
                                               ...
                                               FROM (SELECT patientARVprog.patient_id                                              AS patientID,
                                                            IF(
                                                              ...
                                                              patientARVprog.date_created)                                         AS "dateCreatedOnTB",
                                                     ...
                                                     FROM patient_program patientARVprog
                                                            ...
                                                            LEFT JOIN concept_view cv
                                                              ON progWorkflowState.concept_id = cv.concept_id AND cv.retired = 0
                                                            INNER JOIN visit v
                        ...
                        LEFT JOIN (SELECT Tbpre.pid,
                                          (CASE
                                             WHEN (S1 IN ("Nouveau patient") OR S1 IN ("Traité précédemment"))
                                                     THEN "Oui"
                                             ELSE NULL END)  AS "TbPrecedentsForm",
                                   ...
                                   FROM (SELECT firstAddSectionDateConceptInfo.person_id      AS pid,
                                                ...
                                                (CASE
                                                   ...
                                                   ELSE o3.obs_datetime END)                  AS "obsDateCreated",
                                                (SELECT NAME
                                                 FROM concept_name
                                                 WHERE concept_id = o3.value_coded
                                                   AND locale = 'fr'
                                         ...
                                         FROM (SELECT o2.person_id,
                                                     ...
                                                     FROM obs o
                                                            INNER JOIN concept_name cn
                                                              ON o.concept_id = cn.concept_id AND
                                                                 cn.name IN ('Informations TB') AND
                                                                 cn.voided IS FALSE AND
                                                                 cn.concept_name_type = 'FULLY_SPECIFIED' AND
                                                                 cn.locale = 'fr' AND o.voided IS FALSE
                                                            INNER JOIN encounter e
                                               ...
                                               GROUP BY latestVisitEncounterAndVisitForConcept.visit_id) firstAddSectionDateConceptInfo
                                                INNER JOIN obs o3 ON o3.obs_group_id =
                                                                     firstAddSectionDateConceptInfo.firstAddSectionObsGroupId AND
                                                                     o3.voided = 0 AND o3.void_reason IS NULL
                                                INNER JOIN concept_name cn2 ON cn2.concept_id = o3.concept_id AND
                                                                               cn2.name IN ("Traitement TB Antérieur") AND
                                                                               cn2.voided IS FALSE AND
                                                                               cn2.concept_name_type = 'FULLY_SPECIFIED' AND
                                                                               cn2.locale = 'fr')Tbpre
                                                                 ...
                                                                 FROM (SELECT MAX(o.encounter_id) AS latestEncounter,
                                                                       ...
                                                                       GROUP BY e.visit_id) latestVisitEncounterAndVisitForConcept
                                                                        ...
                                                                        INNER JOIN encounter e2
                                                                          ON o2.encounter_id = e2.encounter_id AND
                                                                             e2.visit_id = latestVisitEncounterAndVisitForConcept.visit_id AND
                                                                             e2.voided IS FALSE
                                                                 GROUP BY latestVisitEncounterAndVisitForConcept.visit_id) firstAddSectionDateConceptInfo
                                                                  INNER JOIN obs o3 ON o3.obs_group_id =
                 ...
                 WHERE COALESCE(tbFormValue.pid, tbPatients.patientID) IS NOT NULL/*removing the patient which dont have program data and form data*/) AS tbnew
         ON tbnew.pid = admdate.person_id
       LEFT JOIN (/* get arv details for each visit of patients  */
                 SELECT arvPatients.person_id       AS pid,
                        (CASE
                           WHEN ifnull(programPhaseCreate, '1981-01-01') > ifnull(obsDateTime, '1981-01-01')
                                   THEN phaseOfProg
                           ELSE LigneARV END)       AS "ligneARV",
                        (CASE
                           WHEN ifnull(programPhaseCreate, '1981-01-01') > ifnull(obsDateTime, '1981-01-01')
                                   THEN phaseDate
                           ELSE Datedébutligne END) AS "Datedébutligne",
                       ...
                       FROM person
                              LEFT JOIN (SELECT Y.program_id,
                                         ...
                                         FROM (SELECT X.program_id,
                                               ...
                                               GROUP BY phaseOfProg, patientID
                                               ORDER BY phaseDate DESC) Y
                                         GROUP BY patientID) arv ON person.person_id = arv.patientID) AS arvPatients
                        LEFT JOIN (SELECT Tbpre.pid,
                                          (CASE
                                             WHEN S1 IN ('1ere',
                                                         '2e',
                                   ...
                                   FROM (SELECT firstAddSectionDateConceptInfo.person_id AS pid,
                                         ...
                                         FROM (SELECT o2.person_id,
                                                      ...
                                                      latestVisitEncounterAndVisitForConcept.concept_id
                                               FROM (SELECT Y.latestEncounter, Y.person_id, Y.concept_id, Y.visit_id
                                                                 ...
                                                                 FROM obs o
                                                                        INNER JOIN concept_name cn
                                                                          ON o.concept_id = cn.concept_id
                                                                               AND
                                                                             (cn.name IN ("Regime actuel") OR cn.name IN ("Regime Debut"))
                                                                               AND cn.voided IS FALSE AND
                                                                             cn.concept_name_type = 'FULLY_SPECIFIED'
                                                                               AND cn.locale = 'fr' AND
                                                                             o.voided IS FALSE
                                                                        INNER JOIN encounter e
                                                                          ON e.encounter_id = o.encounter_id AND e.voided IS FALSE
                                                                 GROUP BY o.person_id, o.encounter_id) AS X
                                                                       ...
                                                                       FROM (SELECT X.latestEncounter,
                                                                             ...
                                                                             FROM (SELECT MAX(o.encounter_id) AS latestEncounter,
                                                                                   ...
                                                                                   FROM obs o
                                                                                          INNER JOIN concept_name cn
                                                                                            ON o.concept_id = cn.concept_id AND
                                                                                               (cn.name IN ("Regime actuel") OR cn.name IN ("Regime Debut")) AND
                                                                                               cn.voided IS FALSE AND
                                                                                               cn.concept_name_type = 'FULLY_SPECIFIED' AND
                                                                                               cn.locale = 'fr' AND
                                                                                               o.voided IS FALSE
                                                                  ...
                                                                  INNER JOIN obs o3 ON o3.obs_group_id =
                                                                                       ...
                                                                                       (SELECT concept_id
                                                                                        ...
                                                                                        WHERE cn2.name IN ("HA, Date début")
                                                                                          ...
                                                                                          AND cn2.concept_name_type = 'FULLY_SPECIFIED'
                                                                                          AND cn2.locale = 'fr'))anne)totalform
                                            ON totalform.PID = Tbpre.pid) formData
                          ON arvPatients.person_id = formData.pid
                 WHERE COALESCE(formData.pid, arvPatients.patientID) IS NOT NULL/*removing the patient which dont have program data and form data*/) arvnew
         ON arvnew.pid = admdate.person_id
       LEFT JOIN (/* get cd4 details for each visit of patients  */
                 ...
                 GROUP BY vid) AS CD ON CD.PID = admdate.person_id AND CD.vid = admdate.visitid
       LEFT JOIN (/* get Syndrome à la sortie details for each visit of patients  */
                 ...
                 FROM (SELECT o2.person_id,
                       ...
                       FROM (SELECT MAX(o.encounter_id) AS latestEncounter, o.person_id, o.concept_id, e.visit_id
                             FROM obs o
                                    INNER JOIN concept_name cn
                                      ON o.concept_id = cn.concept_id AND cn.name IN ("Syndrome à la sortie") AND
                                         cn.voided IS FALSE AND cn.concept_name_type = 'FULLY_SPECIFIED' AND
                                         cn.locale = 'fr' AND o.voided IS FALSE
                                    INNER JOIN encounter e ON e.encounter_id = o.encounter_id AND e.voided IS FALSE
                             GROUP BY e.visit_id) latestVisitEncounterAndVisitForConcept
         ...
         ON siautre.person_id = admdate.person_id AND siautre.visitid = admdate.visitid
GROUP BY pi.identifier, admdate.visitid
