...
group by patient_id,visit_id,enrolledDate
    ...
    LEFT JOIN (SELECT
                                   v.visit_id,
                                   max(hemo_obs.obs_datetime) AS cd4_obsDateTime,
                               ...
                               FROM visit v INNER JOIN encounter e
                                   ...
                                   (SELECT
                                         ...
                                         o.value_numeric,
                                         o.concept_id
                                     FROM obs o INNER JOIN concept_view hemo_cd4
                                             ON hemo_cd4.concept_id = o.concept_id AND o.voided IS FALSE AND hemo_cd4.retired IS FALSE AND
                                                hemo_cd4.concept_full_name IN ('Hémoglobine (Hemocue)(g/dl)') AND o.value_numeric IS NOT NULL
                                    ...
                                    (SELECT
                                     ...
                                     FROM obs o INNER JOIN concept_view hemo_cd4
                                             ON hemo_cd4.concept_id = o.concept_id AND o.voided IS FALSE AND hemo_cd4.retired IS FALSE AND
                                                hemo_cd4.concept_full_name IN ('Hemoglobine') AND o.value_numeric IS NOT NULL)) hemo_obs
                                       ON hemo_obs.encounter_id = e.encounter_id
                               GROUP BY v.visit_id) hemoglobin ON hemoglobin.PID = v.patient_id AND hemoglobin.visit_id = v.visit_id
    LEFT JOIN (SELECT
                                   v.visit_id,
                               ...
                               GROUP BY v.visit_id) glyceme ON glyceme.PID = v.patient_id AND glyceme.visit_id = v.visit_id
                       left join
                       (
                               select
                   o.person_id,
...
where  date(v.date_created) between Date('#startDate#') AND ('#endDate#')  group by v.visit_id  ;
