...
FROM (SELECT to_char(B.date_of_results, 'DD/MM/YYYY')                                        AS "result_date",
                                                 ...
                                                 FROM (/*Pivoting the table row to column*/ SELECT sample.collection_date                                    AS sample_date,
                                                                                            ...
                                                                                            FROM patient_identity pi
                                                                                                   ...
                                                                                                   LEFT JOIN DICTIONARY d
                                                                                                     ON r.result_type = 'D' AND CAST(d.id AS TEXT) = r.value
      ...
      ORDER BY B.date_of_results,
               ...
               B.dob,
               B.sexe) AS A
WHERE COALESCE("K", "Na", "Cl", "Gamma GT(GGT)", "Phosphatase alchaline(Pal)", "Billi Total(BT)", "Billi direct(BD)",
               "Calcium (Ca)") IS NOT NULL;
