...
FROM
  ...
  LEFT JOIN person_attribute pa ON sortdate.person_id = pa.person_id
  AND pa.voided = 0
  ...
  ) AS mode_sortie ON mode_sortie.person_id = sortdate.person_id
  ...
  LEFT JOIN (
    ...
    FROM
      ...
      AND cv.concept_full_name = "Syndrome d'admission"
      AND obsForActivityStatus.voided = 0
  ) AS syndrome ON syndrome.person_id = sortdate.person_id
  AND syndrome.visit = sortdate.visitid
  LEFT JOIN (
    ...
    GROUP BY
      v.visit_id
  ) AS admdate1 ON admdate1.person_id = sortdate.person_id
  AND admdate1.visitid = sortdate.visitid
  LEFT JOIN (
  ...
  ) AS dg ON dg.person_id = sortdate.person_id
  ...
  ) AS dg2 ON dg2.person_id = sortdate.person_id
  ...
  LEFT JOIN (
    ...
    GROUP BY
      visit_id
  ) AS cv ON cv.patient_id = sortdate.person_id
  AND cv.visitid = sortdate.visitid
  LEFT JOIN (
    ...
    GROUP BY
      vid
  ) cd4 ON cd4.pid = sortdate.person_id
  AND cd4.vid = sortdate.visitid
GROUP BY
  pi.identifier,
  sortdate.visitid;
