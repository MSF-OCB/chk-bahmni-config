SELECT date(B.sample_date) :: DATE AS "Date prélèvement",
       B.care_center_requesting AS "Provenance",
       B.Patient_Name AS "Nom",
       B.Patient_Identifier AS "ID",
       concat((current_Date-b.dob)/365 ,' ','Années' , ' ',((current_date-b.dob)%365)/30, ' ','Mois')as Age,
       B.sexe AS "Sexe",
       date(B.date_of_results) AS "Date resultats",
       ...
       sum(cast(B.Hemoglobine_pr AS NUMERIC)) AS "Hb",
