SELECT pi.identifier                                          AS 'ID',
       ...
       CASE
         WHEN DATE(p.date_created) = DATE(v.date_started)
                 THEN 'Nouvelle visite'
         ELSE NULL END                                        AS 'Nouvelle visite',
       (CASE
          WHEN vt.name IN('LAB VISIT') THEN NULL
          ELSE consultant_name.names END)                     AS 'Consultant',
...
FROM visit v
       ...
       LEFT JOIN (SELECT person_id, cv.concept_full_name
                  ...
                  WHERE pat.name = 'Type de cohorte') typeDCohorte ON typeDCohorte.person_id = pi.patient_id
       LEFT JOIN (SELECT person_id, pa.value
                  FROM person_attribute pa
                         INNER JOIN person_attribute_type pat
                           ON pat.person_attribute_type_id = pa.person_attribute_type_id AND pa.voided IS FALSE AND
                              pat.retired IS FALSE
                  WHERE pat.name = 'Date entrée cohorte') dateEntreeCohore ON dateEntreeCohore.person_id = pi.patient_id
       LEFT JOIN (/*getting consulation name*/
                 ...
                 GROUP BY patient_id, v.visit_id) consultant_name ON consultant_name.visit_id = v.visit_id
       LEFT JOIN (/*getting appointment date of the previous visit*/
                 SELECT obs.person_id, obs.value_datetime, latest_obs.visit_id
                 FROM obs obs
                        ...
                        INNER JOIN concept_view cv ON cv.concept_id = obs.concept_id
                                                        AND cv.concept_full_name IN ('Date de prochain RDV',
                                                                                     'Date du prochain RDV') AND
                                                      cv.retired IS FALSE
                        INNER JOIN (SELECT pv.visit_id, pv.prev_visit_id, MAX(o.obs_datetime) AS obsDateTime
                                    FROM obs o
                                           INNER JOIN encounter e
                                             ON e.encounter_id = o.encounter_id AND o.voided IS FALSE AND
                                                e.voided IS FALSE
                                           INNER JOIN concept_view cv ON cv.concept_id = o.concept_id AND
                                                                         cv.concept_full_name IN ('Date de prochain RDV',
                                                                                                  'Date du prochain RDV') AND
                                                                         cv.retired IS FALSE
                                           INNER JOIN (/*Getting previous visit*/
                                                      ...
                                                      FROM visit v
                                                             LEFT JOIN visit prev_visit
                                                               ON DATE(v.date_started) > DATE(prev_visit.date_started)
                                    ...
                                    GROUP BY pv.visit_id) latest_obs ON latest_obs.prev_visit_id = en.visit_id AND
                                                                        latest_obs.obsDateTime = obs.obs_datetime) prev_appt_date
